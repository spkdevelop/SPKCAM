...
def save_image(file_path,objectsToCapture):
    ...
    try:
        ...
        rs.Wallpaper("Superior", PATH_WALLPAPER)
        rs.Wallpaper("Perspectiva", PATH_WALLPAPER)
...
class SettingsControl():
    ...
    def addControls(self):
        ...
        p.addLabel("", "//Sube tu Cogido-G a ", (50, 50, 50), False)
        p.addLinkLabel("","SPKCLoud", "http://www.spkautomatizacion.com/my-cloud",True, None)
        p.addSeparator("sep2", 126, True)
    ...
    def button0_OnButtonPress(self,sender,e):
        ...
        line = {}
        nombre = rs.StringBox ("Nombre base de datos:",self.activecycle,"Guardar rutina en base de datos")
        if nombre in self.cycles:
            nombre = nombre + "_Copia"
        itemdata = self.settings[self.activecycle]
        if isinstance(itemdata, dict) != True:
            itemdata = eval(itemdata)
        line[nombre] = json.dumps(itemdata)
    ...
    def process_objects(self):
        ...
        for point in self.points:
            try:
                ...
                rs.MessageBox("Hay un error en uno de tus puntos. \n\n Revisa bien tu archivo antes de cortar.")
        ...
        for curve in self.curves_open:
            try:
                ...
                rs.MessageBox("Hay un error en una de tus curvas de grabado. \n\n Revisa bien tu archivo antes de cortar.")
        ...
        for curve in self.curves_pocketing:
            try:
                ...
                rs.MessageBox("Hay un error en una de tus curvas de caja. \n\n Revisa bien tu archivo antes de cortar.")
        ...
        for curve in self.curves_inside:
            try:
                ...
                rs.MessageBox("Hay un error en una de tus curvas interiores. \n\n Revisa bien tu archivo antes de cortar.")
...
def Main():
    ...
    if objects:
        ...
        if result == System.Windows.Forms.DialogResult.OK:
            ...
            new_batch = batch(objects,sec_plane,ui)
            if new_batch.upload:
                
                
                spkuser = rs.GetDocumentData(REGISTRY_SECTION,"spkuser")
                spkid = rs.GetDocumentData(REGISTRY_SECTION,"spkid")
                spkname = rs.GetDocumentData(REGISTRY_SECTION,"spkname")
                spkgname = rs.GetDocumentData(REGISTRY_SECTION,"spkgname")
                
                if not spkname:
                    spkname = rs.DocumentName().replace(".3dm","")
                if not spkgname:
                    spkgname = rs.DocumentName().replace(".3dm","_CodigoG")
                
                new_upload = rs.MessageBox("Crear un producto nuevo en la nube?",4,"Sube tu archivo a la nube")
                
                if new_upload == 7:
                    input_data = rs.PropertyListBox(["Correo usuario:","ID Producto:","Nombre del CodigoG:"],[spkuser,spkid,spkgname],"Sube tu archivo a la nube","Ingresa la informacion del producto.")
                    if not input_data:
                        return
                    rs.SetDocumentData(REGISTRY_SECTION,"spkuser",input_data[0].lower().strip())
                    rs.SetDocumentData(REGISTRY_SECTION,"spkid",input_data[1].lower().strip())
                    rs.SetDocumentData(REGISTRY_SECTION,"spkgname",input_data[2].lower().strip())
                else:
                    input_data = rs.PropertyListBox(["Correo usuario:","Nombre del Producto:","Nombre del CodigoG:"],[spkuser,spkname,spkgname],"Sube tu archivo a la nube","Ingresa la informacion del producto.")
                    if not input_data:
                        return
                    rs.SetDocumentData(REGISTRY_SECTION,"spkuser",input_data[0].lower().strip())
                    rs.SetDocumentData(REGISTRY_SECTION,"spkname",input_data[1].lower().strip())
                    rs.SetDocumentData(REGISTRY_SECTION,"spkgname",input_data[2].lower().strip())
            ...
            if file_path:
                ...
                if new_batch.upload:
                    if new_upload == 7:
                        upload_to_server(file_path,settings_path,image_path,image_path_2,input_data[0],new_batch.price,input_data[1],"False",input_data[2])
                    else:
